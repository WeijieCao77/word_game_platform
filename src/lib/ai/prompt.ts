...
12. 推理/调查类（story——本格山庄、都市悬案、密室、探案皆此法，创作者说「想做推理游戏」就按这套搭）：
   ...
   - **关键词搜索玩法**（网页调查游戏的灵魂，创作者说「想做 MISSING/档案调查那种」就用它）：
     把关键信息（人名/地名/事件）埋在前面的文档卡文案里，检索枢纽卡配 input——玩家必须自己读出线索
     再亲手输入才解锁新页面，比点选项上头得多；answers 可带 condition（要先拿到某线索才认这个词），
     配一个耐人寻味的 fallbackText（「档案库里没有这个词条」）；重要关键词给 2~3 个同义写法
