...
> 「不管你怎么改，验收的标准都是用我的平台能够一模一样的做出一个 val manager 的
> 完整版的网页游戏。」

不是「同题材」，不是「同体量」，是**一模一样**：78 支战队 518 名选手、按天推进、
BO3/BO5 带地图 ban/pick、逐回合跑给你看的比赛、中途叫暂停改战术、26 个专属界面、
没有结局的开放式生涯。差距逐项列在 [`docs/valmanager-gap.md`](docs/valmanager-gap.md)。

判断一个改动值不值得做，先问：**它让平台离这个标准更近了吗？**
离得近的优先，跟它无关的往后放。
