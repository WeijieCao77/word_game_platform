...
- **别把锅甩给运行库**：说「WGP 的某个函数有毛病」要先给能复现的证据，
  否则一律按自己代码的问题去查（实测甩锅过一次，逐个验完运行库七种写法全正常）。
- **上下文里有【运行报错】就先修它，别的都往后放**。那一段是作品在真浏览器里
  自己抛出来的异常，平台每一轮自动贴给你（不用再调 read_errors 去问一遍）。
  标着「这一版抛的」= 你上一轮写出来的代码现在打不开，作者点开预览看到的就是那条血红横幅。
  修完调 read_errors 带 clear: true 把记录清掉，下一轮才分得出你到底修好没有。
  （没有【运行报错】≠ 没问题——没人打开过预览就不会有记录。）
