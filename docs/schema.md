...
- **sim 调度器（已上线）**：实体（entityTypes/entities，属性+标签）、派生值、
  决策（actions：目标选择/条件/次数/效果）、结算（settlement：data 赛程行 + compute
  中间量 + outcomes 分支）、曲线（按回合/周期批量规则）、实体事件卡（scope 绑定 self）、
  聚合函数（`avg("选手","枪法","主力")`）、tag()/target./self./row. 表达式扩展、
  resetEachCycle 变量。回合管线：玩家主动决策（可多个）→ 结束回合 → 结算 → 事件 →
  曲线 → 结局检查 → 周期滚动。官方示例《无畏契约经理》（原电竞经理 Lite）即压力测试样板
- **v1.5 · sim 增强**：**阶段进度**（换地图，带各自的对手池/事件池/招募池）、
  **实体运行时生成**（无限青训/天骄池——当前转会用预置候选池+标签流转表达）
