...
老板后来补了两句，把这把尺子说得更准，两句都要一起读：

> 「你就是要用你自己编出来的 prompt 题材更改做出来一个几乎一样的**体验**的游戏，
> 用户感觉是一样的就行，代码层面用户也看不到。但是之前做的完全就是两个东西。」
>
> 「我还是一样的要求，**一定要用平台去做**，要保证你现在做的游戏平台也能做出来。」

拆开就是三条，缺一条都不算过：

1. **量的是玩家的体验，不是代码**。题材名字可以是我们自己的——联盟、战队、选手叫什么都行。
   玩家看不到代码，他看到的是：每天要做几件事、比赛能不能一回合一回合看着打、
   选手会不会因为坐板凳而不信任你、一个赛季有多长、界面上有多少东西可以点。
2. **深度和丰富度一分不能少**。78 支战队 518 名选手、按天推进、BO3/BO5 带地图 ban/pick、
   逐回合跑给你看的比赛、中途叫暂停改战术、26 个专属界面、没有结局的开放式生涯。
   规模本身就是体验的一部分——转会市场上有五百个人可挑和只有二十个人可挑，是两种游戏。
3. **必须由平台的 AI 做出来**。这一条是前两条的前提：我（接手的人或 AI）**一行游戏代码都不许自己写**。
   游戏的每个文件都必须是线上 AI 通过 `/api/games/:id/assistant` 写进去的。
   手搓一份塞进去，测的就不是平台了——那等于自欺。我的活是**修平台**，直到它的 AI 做得出来。

**反面参照**（之前真翻过的车）：用平台做过两款「电竞经理」，共享的只有题材词——
都叫电竞经理、都有选手和赛季，但玩家坐下来玩的是两个完全不同的东西：
原作按天推进、有 ban/pick、有逐回合直播、有 26 个专属界面；那两款是每周点三下、
一次结算给个比分、所有游戏长一个样。差距逐项列在
[`docs/valmanager-gap.md`](docs/valmanager-gap.md)。

判断一个改动值不值得做，先问：**它让平台离这个标准更近了吗？**
离得近的优先，跟它无关的往后放。

**怎么量**：`.github/workflows/valmanager-e2e.yml`（手动触发）。它把
`templates/valmanager/spec.md`（体验说明书）发给线上 AI，把真实世界数据传进作品，
分十几轮驱动它搭，然后报三样：每轮的成长曲线、界面体检（每一页点进去有没有真东西）、
冒烟检查。workflow 只上传 `data/*.csv`，**代码文件一个都不传**——那是 AI 的活。
